...
```{r, include=FALSE}
# Necessary Libraries
pacman::p_load(agridat, tidyverse, dplyr, here, reshape2, # data import and handling
...
`(1|rep)` represents the block level variance - random effect
`(1|rep:mainplot)` represents the whole-plot (main plot) error - random effect
The residual represents the sub-plot variability not captured by mainplot variance
...
```{r}
cor.AR1  <- cov2cor(VarCorr(mod.AR1)$cond$plot)
cor.hCS  <- cov2cor(VarCorr(mod.hCS)$cond$plot)
cor.Toep <- cov2cor(VarCorr(mod.Toep)$cond$plot)
cor.UN <- cov2cor(VarCorr(mod.UN)$cond$plot)

melt_cor <- function(cor_mat, model_name) {
  melt(cor_mat, varnames = c("Week1", "Week2"), value.name = "Correlation") %>%
    mutate(Model = model_name)
}

df_cor <- bind_rows(
  melt_cor(cor.AR1,  "AR(1)"),
  melt_cor(cor.hCS,  "Heterogeneous CS"),
  melt_cor(cor.Toep, "Toeplitz"),
  melt_cor(cor.UN,   "UN"))


ggplot(df_cor, aes(Week1, Week2, fill = Correlation)) +
  geom_tile(color = "white") +
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(
    title = "Random-effect correlation structure across models",
    fill = "r"
  )
