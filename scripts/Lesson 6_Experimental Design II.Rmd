...
```{r}
dataURL <- "https://raw.githubusercontent.com/Conorfair/CAES_Training_Course_Data_Analysis_in_R/refs/heads/main/Split_Plot_Design.csv"
...
RCBD_Plot_tukey<-ggplot() +
  ...
  geom_text(data=all_mean_comparisons_tukey_re,aes(y=10000,x=N_G,label=str_trim(.group)),color="red",
            position=position_nudge(x=-0.2),hjust=0,angle=90)+ # red letters 
...
withinWeek_UN_Plot_tukey<-ggplot() +
  ...
  geom_text(data=withinWeek_mean_comparisons_tukey_UN,aes(y=emmean,x=variety,label=str_trim(.group)),color="red",
...
within_means_tukey_Plot<-ggplot(dat,aes(y=weight,x=trt))+
  ...
  geom_errorbar(data=within_means_tukey,aes(ymin=exp(lower.CL),ymax=exp(upper.CL),x=trt),
                color="red",width=0.1,position=position_nudge(x=0.2))+# red error bars representing the confidence limits of the adjusted means
  geom_text(data=within_means_tukey,aes(y=exp(weight),x=trt,label=str_trim(.group)),color="black",
            position=position_nudge(x=0.3),hjust=0)+ # red letters 
