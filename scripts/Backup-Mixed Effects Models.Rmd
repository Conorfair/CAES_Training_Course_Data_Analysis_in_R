...
Mixed models are known by a variety of names that is usually sub-field specific. The most common terms you may come across are:
* Variance components
...
```{r}
#Loading the Data
x <- "https://raw.githubusercontent.com/Conorfair/CAES_Training_Course_Data_Analysis_in_R/refs/heads/main/Random_Intercept_Model.csv"
data <- read.csv(x)
...
5) Fixed effects are estimated using least squares (or, more generally, maximum likelihood) and random effects are estimated with shrinkage (e.g., linear unbiased prediction).
...
```{r}
dataURL <- "https://raw.githubusercontent.com/Conorfair/CAES_Training_Course_Data_Analysis_in_R/refs/heads/main/Multiple_Random_Intercept_Model.csv"
...
withinG_RCBD_Plot_tukey<-ggplot() +
  facet_wrap(~G,labeller=label_both)+ #facete per G level
  ...
  geom_text(data=withinG_mean_comparisons_tukey_re,aes(y=emmean,x=N,label=str_trim(.group)),color="red",
...
```{r}
x <- "https://raw.githubusercontent.com/Conorfair/CAES_Training_Course_Data_Analysis_in_R/refs/heads/main/Random_Slope_and_Intercept%20Model_Example.csv"
data <- read.csv(x)
str(data)
...
FE_RE_Plot<-ggplot(data,aes(x=NAP,y=Richness,color=Exposure))+
  geom_point(size=5)+
  geom_line(data=new_data,aes(x=NAP,y=Richness,color=Exposure))+
...
```{r}
#Loading the Data
x <- "https://raw.githubusercontent.com/Conorfair/CAES_Training_Course_Data_Analysis_in_R/refs/heads/main/Random_Slope_and_Intercept_Model.csv"
data <- read.csv(x)
str(data)
