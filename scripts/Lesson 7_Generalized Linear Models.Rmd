...
```{r, include=FALSE}
#Necessary Libraries
pacman::p_load(agridat,tidyverse,dplyr,car, # data import and handling
...
```{r}
x<-"https://raw.githubusercontent.com/Conorfair/CAES_Training_Course_Data_Analysis_in_R/refs/heads/main/Count_Model.csv"
Count_Data<-read.csv(x)

Count_Data<-Count_Data%>%filter(spray=='A'|spray=='B'|spray=='C'|spray=='F')%>%
  droplevels()
...
```{r}
x<-"https://raw.githubusercontent.com/Conorfair/CAES_Training_Course_Data_Analysis_in_R/refs/heads/main/Overdispersion.csv"
data<-read.csv(x)
...
```{r}
x<-"https://raw.githubusercontent.com/Conorfair/CAES_Training_Course_Data_Analysis_in_R/refs/heads/main/Logistic_Regressionl.csv"
data<-read.csv(x)
...
ggplot(data,aes(x=field,y=disease_num))+
  ...
  geom_errorbar(data=means,
                ...
                width=.1,lwd=1,position=position_dodge(width=0.5)) +
  geom_point(data=means,aes(x=field,y=prob), 
             size=2,position=position_dodge(width=0.5)) +
    geom_text(data=means,aes(y=prob,x=field,label=str_trim(.group)),color="black",
            position=position_dodgenudge(width=1.2,x=-0.15),hjust=0,angle=0)+ 
  theme_classic()
```
