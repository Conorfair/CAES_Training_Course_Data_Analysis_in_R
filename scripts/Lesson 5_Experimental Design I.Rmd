...
author: "Conor Fair"
date: "2025-10-3"
# Modifications to YAML - "Yet Another Markdown Language"
# Specify output formats and options for each document type (e.g., PDF, MS Word, HTML)
output:
...
```{r, include=FALSE}
#Necessary Libraries
pacman::p_load(agridat, tidyverse,dplyr, # data import and handling
...
When designing experiments, one of the more consequential decisions is how treatments are assigned experimental units. We mentioned Gauss-Markov assumptions and that some assumptions deal with how the experiment is designed. The relevant assumption here is that experimental units are randomly assigned to treatments. When researchers don't always have the tools or resources to meet this assumption, but thankfully there are methods to address this. When this assumption is met, the experiment is said to follow a completely randomized design (CRD).
...
```{r}
dataURL <- "https://raw.githubusercontent.com/Conorfair/CAES_Training_Course_Data_Analysis_in_R/refs/heads/main/Completely_Randomized_Design.csv"
dat <- read.csv(dataURL)
...
CRD_Plot_tukey<-ggplot() +
  ...
  geom_errorbar(data = mean_comparisons_tukey,aes(ymin=lower.CL,ymax=upper.CL,x=variety),color="red",width=0.1,
                position=position_nudge(x=0.1))+
  # red letters 
  geom_text(data=mean_comparisons_tukey,aes(y=emmean,x=variety,label=str_trim(.group)),color="red",
...
```{r}
dataURL <- "https://raw.githubusercontent.com/Conorfair/CAES_Training_Course_Data_Analysis_in_R/refs/heads/main/Randomized_Complete_Block_Design.csv"
dat <- read.csv(dataURL)
```
...
We now produce a field layout of the trial using the desplot() function just like in the CRD above. Notice how the nitrogen and genotype treatments are randomly assigned within each rep (block). Each level of nitrogen and genotype are represented within each block (complete).
...
We see a significant interaction effect between nitrogen and genotype so we won't be testing the main effects independently. Also the rep (block) effect isn't significant.
...
withinG_RCBD_Plot_tukey<-ggplot() +
  ...
  geom_text(data=withinG_mean_comparisons_tukey_fe,aes(y=emmean,x=N,label=str_trim(.group)),color="red",
            position=position_nudge(x=0.2),hjust=0)+ # red letters 
...
When the experimental design is arranged in a grid or matrix with the same number of rows and columns. Each treatment combination appears exactly once in every row and column. This allows for the control of unexplained variation in two directions (i.e., along the rows and columns). 

#### Review the Experiemental Design

Data published from (Bridges 1989) include trials of yield from four different cucumber varieties with four rows and columns. The allocation of the treatments (varieties) to the experimental units (plots) was completely at random along the rows and columns - following a latin square design (LSD). We are reviewing the data collected from one of the two locations.
...
```{r}
dataURL <- "https://raw.githubusercontent.com/Conorfair/CAES_Training_Course_Data_Analysis_in_R/refs/heads/main/Latin_Square_Design.csv"
dat <- read.csv(dataURL)
...
CRD_Plot_tukey_fe<-ggplot() +
  ...
  # red error bars representing the confidence limits of the adjusted means
  geom_errorbar(data = mean_comparisons_tukey_fe,aes(ymin=lower.CL,ymax=upper.CL,x=gen),color="red",width=0.1,
                position=position_nudge(x=0.1))+
  # red letters 
  geom_text(data=mean_comparisons_tukey_fe,aes(y=emmean,x=gen,label=str_trim(.group)),color="red",
            position=position_nudge(x=0.2),hjust=0)+ 
  ylim(0, NA)+ # force y-axis to start at 0
  ylab("Yield in t/ha")+ # label y-axis
  xlab("Variety")+      # label x-axis
...
1) The variation in yield is equal among the four different genotypes
