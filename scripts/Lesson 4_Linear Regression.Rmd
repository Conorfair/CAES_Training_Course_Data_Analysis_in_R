...
```{r, include=FALSE}
#Necessary Libraries
library(dplyr); library(emmeans);library(ggplot2);library(ggpp);
library(multcomp);library(multcompView);library(forcats);library(stringr)
```
...
```{r}
x<-"https://raw.githubusercontent.com/Conorfair/CAES_Training_Course_Data_Analysis_in_R/refs/heads/main/One_Way_ANOVA.csv"
One_Way_Anova<-read.csv(x)
```
...
It is good practice to remove any objects in R's working memory before moving onto another analysis. The dev.off code clears the plots in the Plots window.
...
```{r}
x<-"https://raw.githubusercontent.com/Conorfair/CAES_Training_Course_Data_Analysis_in_R/refs/heads/main/Two_Way_ANOVA.csv"
Two_Way_Anova<-read.csv(x)
...
This link can provide more details for your reference: <https://www.r-bloggers.com/2011/03/anova-–-type-iiiiii-ss-explained/#:~:text=The%20anova%20and%20aov%20functions%20in%20R%20implement%20a%20sequential,levels%20of%20the%20other%20factor.>
...
Plot_all_means_tukey<-ggplot() +
  ...
  geom_text(data=all_means_tukey,aes(y=emmean,x=species,group=sex,label=str_trim(.group)),color="black",
            position=position_dodgenudge(width=1.2,x=-0.025),hjust=0,angle=0)+ # red letters 
...
```{r}
x<-"https://raw.githubusercontent.com/Conorfair/CAES_Training_Course_Data_Analysis_in_R/refs/heads/main/Simple_Linear_Regression.csv"
Simple_Linear_Regression<-read.csv(x)
```
...
```{r}
par(mfrow=c(1,1), mar=c(3.2,3.2,2,.5), mgp=c(2,.7,0)) #Number of rows of figures, the margins of the figure, and margin line for the axis title labels and line
...
```{r}
x<-"https://raw.githubusercontent.com/Conorfair/CAES_Training_Course_Data_Analysis_in_R/refs/heads/main/ANCOVA.csv"
ANCOVA<-read.csv(x)
ANCOVA$loc<-as.factor(ANCOVA$loc)
```
...
mu_density_loc<-mod_2%>%
  emtrends(pairwise~loc,var="density")
mu_density_loc
```
...
Two_Way_Anova<-subset(Two_Way_Anova,!is.na(sex)) #removes NAs within the sex variable
Two_Way_Anova$island<-as.factor(Two_Way_Anova$island)
Two_Way_Anova$year<-as.factor(Two_Way_Anova$year)
Two_Way_Anova$species<-as.factor(Two_Way_Anova$species)
str(Two_Way_Anova)
```
