...
Plot_within_means_tukey <- ggplot() +
  facet_wrap(~ sex, labeller = label_both, scale = "free_y") + # facet per sex level
...
We find the same results that the interaction effect isn't significant, but we can compare against the initial ANCOVA model we fit. We cannot compare against the log- or sqrt-transformed models because they on a different transformed response variable.

```{r ANCOVA Model Comparison}
AIC(mod, mod_2)
```

The polynomial model performed better than the linear ANCVOA model. The residual plot from the log-transformed model looked better than the linear ANCOVA model. Let's consider a model that does both. We can then compare those two models.

```{r ANCOVA log-transformed Model Comparison}
mod_log_2 <- lm(log(yield) ~ poly(density, 2) * loc, data = ANCOVA)
summary(mod_log_2)
autoplot(mod_log_2)

AIC(mod_log, mod_log_2)

car::Anova(mod_log_2, type = "III")
```
...
# First derivative (slopes) at mean density
mu_density_linear <- emtrends(mod_log_2, ~ loc, var = "density", 
                              at = list(density = mean(ANCOVA$density)), deriv = 1)

# Run cld() on the emmGrid object
mu_density_linear_cld <- cld(mu_density_linear, Letters = letters)
mu_density_linear_cld

# Predicted means at mean density (for back-transform)
mu_density_pred <- emmeans(mod_log_2, ~ loc, 
                           at = list(density = mean(ANCOVA$density)))

# Turn both into data frames
df_slopes <- as.data.frame(mu_density_linear_cld)   # includes cld letters
df_preds  <- as.data.frame(mu_density_pred)

# Back-transform slope: slope_log * predicted_yield
df_slopes$predicted_yield <- exp(df_preds$emmean)  # back-transform mean
df_slopes$slope_yield     <- df_slopes$density.trend * df_slopes$predicted_yield
...
# Prediction grid for fitted curves (log model)
new.x <- expand.grid(density = seq(18.78, 184.75, length.out = 100),
                     loc = levels(ANCOVA$loc))
new.y <- predict(mod_log_2, newdata = new.x, interval = "confidence")

addThese <- data.frame(new.x, new.y) %>%
  rename(log_yield = fit) %>%
  mutate(yield = exp(log_yield),
         lwr = exp(lwr),
         upr = exp(upr))

# df_slopes_plot has slope, .group, loc
# df_preds has predicted mean yield, loc

df_slopes_plot <- df_slopes %>% select(loc, density.trend, .group) %>%
  mutate(density = mean(ANCOVA$density))

df_preds <- df_preds %>% select(loc, emmean) %>%
  mutate(predicted_yield = exp(emmean))

# Join by loc to make sure everything lines up
df_slopes_plot <- left_join(df_slopes_plot, df_preds, by = "loc")

# Compute back-transformed slope
df_slopes_plot <- df_slopes_plot %>%
  mutate(slope_yield = density.trend * predicted_yield)

# Define the canonical order once
loc_levels <- c("P", "V")   # or whatever order you want in the legend

# Apply to all data frames
ANCOVA$loc         <- factor(ANCOVA$loc, levels = loc_levels)
addThese$loc       <- factor(addThese$loc, levels = loc_levels)
df_slopes_plot$loc <- factor(df_slopes_plot$loc, levels = loc_levels)

# Base plot: raw data + fitted lines
p <- ggplot(ANCOVA, aes(x = density, y = yield, color = loc)) +
  geom_point(size = 3, alpha = 0.6) +
  geom_ribbon(data = addThese, 
              aes(x = density, ymin = lwr, ymax = upr, fill = loc), 
              alpha = 0.2, color = NA, inherit.aes = FALSE, show.legend = FALSE) +
  geom_line(data = addThese, aes(x = density, y = yield, color = loc), size = 1) +
  scale_color_manual(values = c(P = "blue", V = "red")) +
  scale_fill_manual(values = c(P = "blue", V = "red")) +
  theme_classic(base_size = 14)

# Add CLD labels with matching colors
p <- p + geom_label(
  data = df_slopes_plot,
  aes(x = density, y = predicted_yield, label = str_trim(.group), fill = loc),
  color = "white",       # text color
  fontface = "bold",     # bold letters
  label.size = 0,        # remove border around label
  alpha = 0.8,           # semi-transparent background
  nudge_y = 0.05 * max(ANCOVA$yield),  # move letters above points
  show.legend = FALSE
  )
p
